...
namespace Failsafe {
	...
	class Child : public Child_policy
	{
		...
		public:
			...
			Genode::Semaphore child_request_barrier;
			...
			************************************************/
			...
			Genode::Root_capability child_root_cap;

			Genode::Semaphore child_root_barrier;
	                      			...
	                      			Genode::Server         *) override
			{
				...
				if (Genode::strcmp("Hello", service_name) == 0){
					child_root_cap = root_cap;
					child_root_barrier.up();
				}
			...
			{
				return child_root_cap;
			}

			/****** block until announcement*****/

			void block_for_child_announcement()
			{
				child_root_barrier.down();
			}
